    ...
    while not stopthread.is_set():
        ...
        for i in range(200):
            ...
            if voltageold != voltage:
                voltageold = voltage
                #print(f"Task {task}: Voltage set to {voltage}")
...
class DummyTask:
    ...
    def add_do_chan(self, channel, line_grouping=None):
        print(f"Channel {channel} added with line grouping {line_grouping}")
...
# Dummy DAQ Channels
Pchannel = "Dummy/ai2"  # Pressure channel
mpchannel = "Dummy/line11"  # Main Power Switch
h1channel = "Dummy/line5"  # Heater 1
h2channel = "Dummy/line6"  # Heater 2
h3channel = "Dummy/line7"  # Heater 3

# Dummy DAQ Tasks
mptask = DummyTask("Main Power")
h1task = DummyTask("Heater 1")
h2task = DummyTask("Heater 2")
h3task = DummyTask("Heater 3")

# Main power off by default
mptask.do_channels().add_do_chan(mpchannel)
mptask.start()
mptask.write(False)
mptask.stop()

# Initialize heater tasks
h1task.do_channels().add_do_chan(h1channel)
h2task.do_channels().add_do_chan(h2channel)
h3task.do_channels().add_do_chan(h3channel)
